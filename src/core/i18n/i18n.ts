...
import { loadTranslations, deepMergeWithFallback } from './utils'
 ...
 */
async function getTranslationsForLocale(locale: string): Promise<Record<string, any>> {
  ...
  try {
    ...
    const generatedPath = path.join(process.cwd(), 'public', 'locales', '_generated', `${locale}.json`);
    ...
    try {
      ...
      console.warn(`Pre-built translations not found for ${locale}, falling back to dynamic loading`);
      ...
      if (locale !== defaultLocale) {
        ...
        try {
          ...
          const defaultLocaleFilePath = path.join(localesBasePath, `${defaultLocale}.json`);
          try {
      ...
      } else if (locale !== defaultLocale && Object.keys(defaultMessages).length > 0) {
